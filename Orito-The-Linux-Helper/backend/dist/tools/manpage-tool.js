...
// 5-minute TTL cache for man pages — man page content is static but we want
// to avoid hammering external sources during multi-query sessions
const manpageCache = new TTLCache({
    maxEntries: 200,
    defaultTtlMs: 5 * 60 * 1000, // 5 minutes
});
const MANPAGE_TIMEOUT_MS = 10000; // 10 second timeout
/**
 ...
 */
async function fetchFromManSources(command, section) {
    const sources = [
        // man7.org - comprehensive Linux man pages
        section
            ? `https://man7.org/linux/man-pages/man${section}/${command}.${section}.html`
            : `https://man7.org/linux/man-pages/man1/${command}.1.html`,
        // die.net - alternate source
        `https://linux.die.net/man/${section || 1}/${command}`,
        // tldr.sh for simplified examples
 ...
 */
function parseManPage(html, command, section) {
 ...
 * ```
 */
export async function lookupManpage(command, section) {
 ...
 */
export function clearManpageCache() {
    manpageCache.clear();
}
//# sourceMappingURL=manpage-tool.js.map