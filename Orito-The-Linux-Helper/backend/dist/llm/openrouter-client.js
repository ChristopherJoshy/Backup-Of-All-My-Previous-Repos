...
import { getOptimalParams, shouldEnableTools } from './model-selector.js';
...
const completionCache = new TTLCache({
    maxEntries: config.CACHE_MAX_ENTRIES,
    defaultTtlMs: config.CACHE_TTL_SECONDS * 1000,
});
    ...
    while (attempt < MAX_RETRIES) {
        try {
            ...
            if (attempt >= MAX_RETRIES) {
                ...
                throw new LLMError(`${label} failed after ${attempt} attempts: ${lastError.message}`, attempt, lastError);
            }
...
function formatSystemProfile(profile) {
    ...
    const distro = profile.distro;
    const version = profile.distroVersion ?? profile.version;
    const packageManager = profile.packageManager;
    const shell = profile.shell;
    const desktopEnv = profile.windowManager ?? profile.desktopEnvironment;
    if (!distro) {
        return '';
    }
...
- **System**: ${context.systemProfile ?
        (('distro' in context.systemProfile ? context.systemProfile.distro : context.systemProfile.distro) || 'Not specified') +
            ('distroVersion' in context.systemProfile ? context.systemProfile.distroVersion : context.systemProfile.version || '')
        : 'Not specified'}
 ...
 */
export async function openRouterComplete(messages, options = {}) {
 ...
 */
export async function openRouterStream(messages, options = {}, onChunk) {
    const modelId = options.modelId ?? DEFAULT_MODEL;
    const optimalParams = getOptimalParams(modelId);
    const temperature = options.temperature ?? optimalParams.temperature;
    const maxTokens = options.maxTokens ?? optimalParams.maxTokens;
 ...
 */
export async function openRouterCompleteWithModelSelection(messages, taskContext, options = {}) {
