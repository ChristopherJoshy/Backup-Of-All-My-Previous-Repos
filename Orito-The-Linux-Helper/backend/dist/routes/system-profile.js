...
export async function systemProfileRoutes(app) {
    // GET /api/v1/system-profile - Get user's system profile
    app.get('/api/v1/system-profile', { preHandler: [app.authenticate] }, async (request, reply) => {
        ...
        try {
            ...
            const profile = {
                ...
                distroVersion: prefs.get('distroVersion') || null,
                packageManager: prefs.get('packageManager') || null,
                shell: prefs.defaultShell !== 'not_selected' ? prefs.defaultShell : null,
                desktopEnvironment: prefs.get('desktopEnvironment') || null,
            };
            app.log.info({
