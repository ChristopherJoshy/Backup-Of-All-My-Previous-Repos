...
if (!admin.apps.length) {
    const serviceAccount = JSON.parse(readFileSync(config.FIREBASE_SERVICE_ACCOUNT_PATH, 'utf-8'));
    admin.initializeApp({
...
async function createAuthAuditLog(action, userId, sessionId, details, ipAddress, userAgent) {
    try {
...
export async function authRoutes(app) {
    app.post('/api/v1/auth/firebase', async (request, reply) => {
        ...
        try {
            ...
            const isAdminEmail = decoded.email === config.SUPER_ADMIN_EMAIL;
            if (!user) {
                app.log.info({
                    event: 'user_created',
                    user: {
            ...
            await createAuthAuditLog(isNewUser ? 'signup' : 'login', user._id, session._id, { method: 'firebase', email: user.email, isNewUser }, request.ip, request.headers['user-agent'] || '');
            app.log.info({
    ...
    app.post('/api/v1/auth/anonymous', async (request, reply) => {
        ...
        try {
            ...
            const token = app.jwt.sign({ userId: null, sessionId: session._id.toString(), tier: 'trial', isAdmin: false }, { expiresIn: '24h' });
            app.log.info({
            ...
            if (decoded.userId) {
                ...
                try {
                    ...
                    if (activeOverride && activeOverride.expiresAt <= new Date()) {
                        ...
                        user.tier = activeOverride.originalTier;
                        await user.save();
                    }
                }
