...
export class ResearchAgent extends BaseAgent {
    subResearchCount = 0; // Track sub-research spawns to prevent explosion
    static MAX_SUB_RESEARCH = 1; // Max 1 sub-research per research agent
    constructor(task, context, agentConfig, parentAgentId, depth, modelId) {
        super('research', 'research-agent', '#00d4aa', task, context, agentConfig, parentAgentId, depth, modelId);
    }
    /**
...
- Any warnings or version-specific notes`;
            ...
            ], ['web_search', 'search_wikipedia', 'calculate'], { temperature: 0.7, maxTokens: 4000, maxToolCalls: 5 });
            // Track token usage
            if (usage) {
            ...
            const citations = this.extractCitationsFromToolCalls(toolCalls, maxResults);
            ...
            if (this.depth === 0 && this.subResearchCount < ResearchAgent.MAX_SUB_RESEARCH) {
                ...
                if (identifiedSubTopic) {
                    ...
                    try {
                        const subResult = await this.spawnSubAgent('research', `Deep dive: ${identifiedSubTopic.slice(0, 60)}`, identifiedSubTopic, { conversationContext: data?.conversationContext, dateContext, strategy: 'deep', maxResults: 8 });
     ...
     */
    getMaxResultsForStrategy(strategy) {
        switch (strategy) {
            case 'quick':
                return 3;
            case 'deep':
     ...
     */
    extractCitationsFromToolCalls(toolCalls, maxResults) {
        const citations = [];
        ...
        const uniqueCitations = citations.filter((citation, index, self) => index === self.findIndex(c => c.url === citation.url));
        return uniqueCitations
            .sort((a, b) => b.confidence - a.confidence)
            .slice(0, maxResults);
    }
    // Identify if a sub-topic needs deeper research
    async identifySubTopic(query, citations) {
        ...
        if (query.split(/\s+/).length < 5)
            return null; // Only for complex queries
        if (citations.length < 3)
            return null; // Need enough results to identify sub-topics
        try {
...
If no, reply with "NONE".`;
            ...
            ], { maxTokens: 30, temperature: 0, modelId: this.selectedModel });
            // Track token usage
            if (llmResponse.usage) {
                this.recordTokenUsage(llmResponse.usage.totalTokens);
            }
            const trimmed = llmResponse.content.trim();
            if (trimmed === 'NONE' || trimmed.length < 5 || trimmed.length > 100)
                return null;
            return trimmed;
        }
        catch {
