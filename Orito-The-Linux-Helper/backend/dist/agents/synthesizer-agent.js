...
export class SynthesizerAgent extends BaseAgent {
    ...
    analyzeComplexity(commands, steps) {
    ...
    determineResponseType(commands) {
        ...
        const hasRepairKeywords = commands.some(cmd => /fix|repair|restore|recover|troubleshoot/i.test(cmd.command));
        return hasRepairKeywords ? 'repair' : 'action';
    }
    formatInteractiveGuide(query, researchSummary, steps, commands, blocked, warnings, suggestions) {
        let guide = `# ${query}\n\n`;
    ...
    async run(query, additionalData) {
        ...
        try {
            ...
            const blockedStr = data.blocked && data.blocked.length > 0
                ? `\n\nBLOCKED commands (DO NOT suggest these):\n${data.blocked.map((b) => `- ${b.command}: ${b.riskExplanation}`).join('\n')}`
                : '';
...
- Use markdown formatting for clarity (headers, lists, code blocks)`;
            ...
            try {
                await this.executeWithRetry(async () => {
                    ...
                    ], { modelId: this.selectedModel }, (chunk) => {
                        content += chunk;
            ...
            const formattedResponse = this.formatInteractiveGuide(query, data.researchSummary, data.steps, data.commands, data.blocked || [], data.warnings || [], data.suggestions || []);
