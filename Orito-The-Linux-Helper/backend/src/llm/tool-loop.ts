...
import { getAllToolSchemas, ToolSchema } from '../tools/tool-schemas.js';
 ...
 */
function hasToolCalls(response: any): response is { tool_calls: LLMToolCall[] } {
 ...
 */
function extractToolCalls(response: any): LLMToolCall[] {
    if (!response) return [];
 ...
 */
function toolResultsToMessages(results: FormattedToolResult[]): Message[] {
 ...
 */
function addAssistantMessageWithTools(
    messages: Message[],
    content: string,
 ...
 */
function addToolResults(messages: Message[], results: FormattedToolResult[]): Message[] {
...
export async function completionWithTools(
    ...
    const allToolCalls: Array<{ name: string; args: Record<string, any>; result: any }> = [];
    ...
    for (let iteration = 0; iteration < maxIterations; iteration++) {
        ...
        if (!validation.valid) {
            ...
            currentMessages = addAssistantMessageWithTools(
                currentMessages,
                response.content,
                toolCalls
            );
            ...
            const errorResults: FormattedToolResult[] = validation.errors.map((error, i) => ({
        ...
        for (let i = 0; i < validation.calls.length; i++) {
            ...
            try {
                ...
                // Track stats
                globalToolStats.record(
                    parsedCall.name,
                    0, // Duration tracked in executor
 ...
 */
export async function simpleCompletionWithTools(
    userMessage: string,
    systemPrompt: string = TOOL_SYSTEM_PROMPT,
    options: CompletionWithToolsOptions = {}
): Promise<CompletionResult> {
 ...
 */
export async function streamingCompletionWithTools(
    messages: Message[],
    tools: ToolSchema[],
    allowedTools: string[],
    modelId: string,
    onChunk: (chunk: string) => void,
    options: Omit<CompletionWithToolsOptions, 'onIteration'> = {}
): Promise<CompletionResult> {
    // First, do the tool loop without streaming
    const result = await completionWithTools(
        messages,
        tools,
        allowedTools,
        modelId,
        { ...options, maxIterations: options.maxIterations || 5 }
    );
 ...
 */
export function selectRelevantTools(message: string): string[] {
