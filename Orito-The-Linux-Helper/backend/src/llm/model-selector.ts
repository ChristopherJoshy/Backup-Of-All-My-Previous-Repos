 ...
 */
function needsDeepReasoning(query: string): boolean {
 ...
 */
function needsCodingModel(query: string): boolean {
 ...
 */
function needsToolHeavyModel(query: string, toolCount: number = 0): boolean {
 ...
 */
function needsLongContext(query: string, estimatedSize: number = 0): boolean {
 ...
 */
export function selectModel(context: TaskContext): ModelSelectionResult {
    const {
        query,
        requiresTools = false,
        toolCount = 0,
    ...
    let confidence: number = CONFIDENCE_WEIGHTS.DEFAULT_FALLBACK;
    let reasoning: string = 'Default model for general agent tasks';
    let estimatedLatency: 'fast' | 'medium' | 'slow' = 'fast';
 ...
 */
export function getOptimalParams(modelId: string): { temperature: number; topP: number; maxTokens: number } {
    try {
 ...
 */
export function shouldEnableTools(modelId: string): boolean {
    try {
        const config = getModelConfig(modelId);
