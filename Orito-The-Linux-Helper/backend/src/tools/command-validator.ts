...
export function validateCommand(command: string, detectedPM: string | null): {
    blocked: boolean;
...
export function getDryRunEquivalent(command: string): string | null {
    if (/\bapt-get\s+install\b/.test(command)) return command.replace('apt-get install', 'apt-get -s install');
    if (/\bapt\s+install\b/.test(command)) return command.replace('apt install', 'apt -s install');
    if (/\bdnf\s+install\b/.test(command)) return command + ' --setopt=tsflags=test';
    if (/\bpacman\s+-S\b/.test(command)) return command.replace('-S', '-Sp');
    return null;
}
