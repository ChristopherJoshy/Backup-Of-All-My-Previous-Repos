 ...
 */
async function fetchWithTimeout(url: string, options?: RequestInit, timeoutMs: number = SEARCH_TIMEOUT_MS): Promise<Response> {
...
async function searchSearXNG(query: string) {
    ...
    if (!response.ok) {
        if (config.TAVILY_API_KEY) return await searchTavily(query);
...
async function searchTavily(query: string) {
    if (!config.TAVILY_API_KEY) throw new Error('No search provider available');

    const response = await fetchWithTimeout(`${toolConfig.search.tavilyApiUrl}/search`, {
        method: 'POST',
