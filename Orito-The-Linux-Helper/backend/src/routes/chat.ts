...
import { tierMiddleware, incrementRequestCountMiddleware } from '../middleware/tier.js';
import { incrementRequestCount } from '../models/usage.js';
import { mongoIdSchema, validateOrThrow } from '../validation/schemas.js';
...
export async function chatRoutes(app: FastifyInstance) {
    ...
    app.post('/api/v1/chats', async (request, reply) => {
        const { userId, sessionId, tier } = request.user as any;
        ...
        try {
            ...
            return reply.code(500).send({ error: 'Failed to create chat', details: err.message });
        }
    });

    app.get('/api/v1/chats', async (request, reply) => {
        const { userId, tier } = request.user as any;
