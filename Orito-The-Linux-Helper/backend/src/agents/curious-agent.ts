...
import type { OrchestratorContext, SystemProfile, AgentQuestion } from '../types.js';
...
const DISCOVERY_COMMANDS: Record<string, { command: string; parser: (output: string) => Partial<SystemProfile> }> = {
    os_release: {
        command: 'cat /etc/os-release',
        parser: (output) => {
            const lines = output.split('\n');
            const get = (key: string) => lines.find(l => l.startsWith(`${key}=`))?.split('=')[1]?.replace(/"/g, '') || null;
...
export class CuriousAgent extends BaseAgent {
    ...
    async run(input: string, additionalData?: Record<string, unknown>): Promise<CuriousAgentOutput> {
        this.startMetrics();
        ...
        try {
            ...
            if (!profile?.distro) missingFields.push('os_release');
            if (!profile?.kernel) missingFields.push('kernel');
            if (!profile?.packageManager) missingFields.push('package_manager');
            if (!profile?.cpuModel) missingFields.push('cpu');
            if (!profile?.gpuInfo) missingFields.push('gpu');
            if (!profile?.shell) missingFields.push('shell');
            if (!profile?.displayServer) missingFields.push('display');
            if (!profile?.windowManager) missingFields.push('wm');
...
After any tool calls, generate a friendly message asking the user to run the discovery commands above. Explain why this information helps us provide better assistance.`;
            ...
            const { content, toolCalls, usage } = await this.callWithTools(
                ...
                ['web_search', 'lookup_manpage'],
                { temperature: 0.7, maxTokens: 500, maxToolCalls: 3 }
            );
                ...
                if (this.canSpawnSubAgent()) {
                    try {
                        await this.spawnSubAgent(
                            'research',
                            `Research: ${this.task.slice(0, 60)}`,
                            this.task,
                            { strategy: 'quick', maxResults: 3 }
                        );
                    } catch (err) {
    ...
    static parseDiscoveryOutput(outputs: Record<string, string>): Partial<SystemProfile> {
        let merged: Partial<SystemProfile> = {};
    ...
    static getRequiredFields(profile: SystemProfile | null): string[] {
        if (!profile) return Object.keys(DISCOVERY_COMMANDS);
        const missing: string[] = [];
        if (!profile.distro) missing.push('os_release');
        if (!profile.kernel) missing.push('kernel');
        if (!profile.packageManager) missing.push('package_manager');
        return missing;
    }
     ...
     */
    async collectProfileViaQuestions(): Promise<SystemProfile | null> {
        try {
            ...
            const profile = await collectSystemProfile(
                ...
                async (questions: AgentQuestion[]) => {
                    return await this.askUserQuestions(questions);
                },
                async (chatId: string, profileData) => {
                    // Save to database
